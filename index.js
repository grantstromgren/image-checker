...
const logLevels = [ 'INFO', 'WARNING', 'DEBUG', 'CRITICAL' ]
...
process.argv.length <= 2 && exit('Command missing. Use --help to view current commands.', 3);
...
/**
 * flags an image or directory of images
 * @param args
 */
function flag(args = []) {
...
/**
 * checks an image or directory of images against the current store.db
 * @param args
 */
function check(args = []) {
    let { files, stats, data } = preload(args);
...
function store(file, data) {
    ...
    try {
        ...
        exit(`Error storing file: ${file.filePath}`, 3, err);
    }
...
function exit(message, level = 0, err = null) {
    ...
    if (err && process.argv.includes('--debug')) {
        log('', 2, err)
    }
...
// Dynamically run the function based on the arguments passed
try {
    eval(process.argv[2])(process.argv.slice(3));
} catch (err) {
    // An error is most likely to happen if the command is not found
    exit(`Command not found: ${process.argv[2]}`, 3, err);
}